...
def PDFScrape(data):
    ...
    for procedure in data["EligibilityServiceTreatmentHistory"]:
        ...
        if limitationcode:
            if "," in limitationcode:               
                codes =limitationcode.split(',')                
                codes_dis=""
                for code in codes:
                    codes_dis+=", "+limitationcontent[code.strip()]

                limitationcode = codes_dis.strip(",")

            elif "." in limitationcode:
                c = limitationcode.strip() 
                limitationcode = limitationcontent[str(int(float(c)))]
            else:        
                c = limitationcode.strip() 
                limitationcode = limitationcontent[c]                


#     with open("newJson_20834.json", "w") as jsonFile:
#         json.dump(data, jsonFile,indent=4)


# with open("20834_output.json", "r") as jsonFile:
...
def main(Scraperdata, request):
    ...
    if EligibilityPatient[0].get("CoordinationofBenefitsType") == "" or EligibilityPatient[0].get("CoordinationofBenefitsType") is None:
    ...
    try:
        for alterbenefit in EligibilityTreatmentHistory:
            if "Composite/Resin " == alterbenefit.get('Procedure'):
                EligibilityPatientVerification.update({"AlternativeBenefitProvision":alterbenefit.get("LimitationORNotes")})
    except:
        pass
...
#     request = json.load(open(r"C:\BPK\Eligibility_Python\VSCODE\26_06\SD Payor Scraping\Missouri.json", 'r'))
#     Scraperdata=json.load(open(r"C:\BPK\Eligibility_Python\VSCODE\26_06\SD Payor Scraping\20834_output.json", 'r'))
#     data = main(Scraperdata,request)
#     with open("MissouriOutput_20834_output.json", "w") as outfile:
