...
def upload_pdf_to_azure(pdf_path, container_name, connection_string):
    try:
        ...
        # Create BlobClient and upload the image
        random_=''.join(random.choices(string.ascii_uppercase +string.digits, k=10))
        blob_client = blob_service_client.get_blob_client(container=container_name, blob=random_+pdf_path)
        ...
        url = f"https://{blob_service_client.account_name}.blob.core.windows.net/{container_name}/{random_}{pdf_path}"
        return url
